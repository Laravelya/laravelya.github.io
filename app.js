...
function terimaDataWiFiFromAndroid(ssid, bssid) {
    ...
    // 1. Bersihkan format (hapus kutip, ubah strip '-' jadi titik dua ':', dan lowercase)
    let cleanedBssid = bssid.replace(/"/g, "").replace(/-/g, ":").trim().toLowerCase();

    // 2. Abaikan jika Android mengembalikan BSSID dummy (efek GPS mati / izin lokasi ditolak)
    if (cleanedBssid === "02:00:00:00:00:00" || cleanedBssid === "<unknown bssid>" || cleanedBssid === "") {
        console.warn("BSSID dummy terdeteksi. Pastikan GPS HP aktif.");
        return;
    }

    let oldBssid = bssidPengguna;
    bssidPengguna = cleanedBssid;

    // 3. Cek perubahan hanya jika oldBssid sudah ada dan bernilai valid
    if (oldBssid && oldBssid !== bssidPengguna) {
        const cameraArea = document.getElementById('cameraArea');
        if (cameraArea && !cameraArea.classList.contains('hidden')) {
            alert(`Peringatan: Jaringan WiFi Anda berubah!\n(Dari: ${oldBssid} ke: ${bssidPengguna})\nPastikan tetap terhubung ke WiFi resmi sekolah.`);
        }
    }
}
